angular.module('starter').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/home/home.html',
    "<ion-view title=\"Compliance\" cache-view=\"false\" class=\"backhroundcolor\" ng-init=\"getMyComplianceStatus();\" ng-controller=\"CTRL_HOME as $ctrl\"><ion-header-bar class=\"bar-positive\"><h1 class=\"title\">Compliance</h1></ion-header-bar><ion-content class=\"padding\"><div><span class=\"userName blockDisplay\">Welcome {{emailAddress}}</span> <span ng-if=\"$ctrl.gotStatus\"><span ng-if=\"!$ctrl.fullComplaint\"><input type=\"checkbox\" name=\"training\" id=\"training\" ng-change=\"iComply()\" ng-model=\"$ctrl.training\" ng-checked=\"$ctrl.training\"><label for=\"constraintCheckBoxTraining\" style=\"color: white\"><b>I checked \"MS portal\" and there is no training pending for me.</b></label><br><br><br><input type=\"checkbox\" name=\"phishing\" id=\"phishing\" ng-change=\"iComply()\" ng-model=\"$ctrl.phishing\" ng-checked=\"$ctrl.phishing\"><label for=\"constraintCheckBoxPhishing\" style=\"color: white\"><b>I do check the sender of the email and I don't click on any link that takes me to non out side MS network. I didn't click any phishing email.</b></label></span><span ng-if=\"$ctrl.fullComplaint\"><img ng-src=\"/ui/img/ok.png\" class=\"Imgcenter\"></span></span></div></ion-content></ion-view>"
  );
  ...
  $templateCache.put('app/reportees/reportees.html',
    "<ion-view title=\"My Team\" class=\"backhroundcolor\" cache-view=\"false\" ng-init=\"getUserDetails();\" ng-controller=\"CTRL_reportees as $ctrl\"><ion-header-bar class=\"bar-positive\"><h1 class=\"title\">Help</h1></ion-header-bar><ion-content class=\"padding\"><div class=\"list list-inset searchbox\"><label class=\"item item-input searchboxInput\"><input type=\"text\" placeholder=\"Add a team member\" ng-model=\"$ctrl.newReportee\" ng-keyup=\"checkEnter()\"></label></div><br><button class=\"button button-calm order-button cashBtn\" ng-click=\"addReportee()\">Add to my team</button><div class=\"topSpace\"><ion-item class=\"smallPadding\" ng-repeat=\"reportee in myReportees track by $index\" on-swipe-left=\"deleteReportee({{$index}})\"><div class=\"productDesc\" style=\"height: 55px\"><span class=\"floatingLeftToDo italicBlue\" style=\"line-height: 1.5\"><b>{{reportee}}</b></span></div></ion-item></div></ion-content></ion-view>"
  );
