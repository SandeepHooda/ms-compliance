angular.module('starter').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/home/home.html',
    "<ion-view title=\"Compliance\" cache-view=\"false\" class=\"backhroundcolor\" ng-init=\"callCxf();\" ng-controller=\"CTRL_HOME as $ctrl\"><ion-header-bar class=\"bar-positive\"><h1 class=\"title\">Compliance</h1></ion-header-bar><ion-content class=\"padding\"><div><span class=\"userName blockDisplay\">Welcome {{emailAddress}}</span></div></ion-content></ion-view>"
  );
  ...
  $templateCache.put('app/reportees/reportees.html',
    "<ion-view title=\"My Team\" class=\"backhroundcolor\" cache-view=\"false\" ng-init=\"getReportees();\" ng-controller=\"CTRL_reportees as $ctrl\"><ion-header-bar class=\"bar-positive\"><h1 class=\"title\">Help</h1></ion-header-bar><ion-content class=\"padding\"><div class=\"list list-inset searchbox\"><label class=\"item item-input searchboxInput\"><input type=\"text\" placeholder=\"Add a tesm member\" ng-model=\"$ctrl.newReportee\" ng-keyup=\"checkEnter()\"></label></div><br><button class=\"button button-calm order-button cashBtn\" ng-click=\"addReportee()\">Add to my team</button><div class=\"topSpace\"><ion-item class=\"smallPadding\" ng-repeat=\"reportee in myReportees track by $index\" on-swipe-left=\"deleteReportee({{$index}})\"><div class=\"productDesc\" style=\"height: 55px\"><span class=\"floatingLeftToDo italicBlue\" style=\"line-height: 1.5\"><b>{{reportee}}</b></span></div></ion-item></div></ion-content></ion-view>"
  );
